...
# NOTE: make_unique creates a new ptr and will invoke foo1's copy constructor:
new Foo(0x7ffeed807048, data=foo1)
copy constructor Foo(0x7fbc78d00030, data=)
delete Foo(0x7ffeed807048, data=foo1)

# NOTE: to avoid the copy, do this:
new Foo(0x7fbc78d00050, data=foo2)

# As you cannot copy unique pointers, reassign it with move

# Let's print all the unique ptrs now
uptr1 = Foo(0x7fbc78d00030, data=foo1)
uptr2 = nullptr
uptr3 = Foo(0x7fbc78d00050, data=foo2)

# Expect the unique ptr data to be destroyed now
delete Foo(0x7fbc78d00050, data=foo2)
delete Foo(0x7fbc78d00030, data=foo1)
</pre>
