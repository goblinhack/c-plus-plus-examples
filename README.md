...
# Create a custom vector class:
new MyVector(0x7ffee0897730, currlen=0, maxlen=1 elems=[])
push_back called MyVector(0x7ffee0897730, currlen=1, maxlen=1 elems=[10])
push_back called MyVector(0x7ffee0897730, currlen=2, maxlen=2 elems=[10,11])
push_back called MyVector(0x7ffee0897730, currlen=3, maxlen=4 elems=[10,11,12])
...
# End, expect vec1 destroy:
delete MyVector(0x7ffee0897730, currlen=3, maxlen=4 elems=[10,11,12])
</pre>
...
# create account1 and try to deposit into it
new cash BankAccount(0x7ffee26e9f68, cash $0)

# invoke a bind to a method via proxy
copy cash constructor called for BankAccount(0x7ffee26e9f68, cash $0)
copy cash constructor result is  BankAccount(0x7ffee26e9f60, cash $0)
deposit cash called BankAccount(0x7ffee26e9f60, cash $100)

# invoke a bind to a method via proxy with std::ref
deposit cash called BankAccount(0x7ffee26e9f68, cash $100)

# invoke a bind to a method directly
deposit cash called BankAccount(0x7ffee26e9f68, cash $200)

# check the deposits succeeded
delete account BankAccount(0x7ffee26e9f60, cash $100)
delete account BankAccount(0x7ffee26e9f68, cash $200)
# FAILED: account1 deposit failed!: account has different funds BankAccount(0x7ffee26e9f68, cash $200) than expected 300
</pre>
...
# create account1 and try to deposit into it
new cash BankAccount(0x7ffee58e5540, cash $0)
deposit cash called BankAccount(0x7ffee58e5540, cash $100)
deposit cash called BankAccount(0x7ffee58e5540, cash $200)
# SUCCESS: account1 1st deposit succeeded!
delete account BankAccount(0x7ffee58e5540, cash $200)
...
# create account1 and try to deposit into it
new cash BankAccount(0x7ffeebfe66b0, cash $0)
deposit cash called BankAccount(0x7ffeebfe66b0, cash $100)
BankAccount(0x7ffeebfe66b0, cash $100)
# SUCCESS: account1 deposit succeeded!
delete account BankAccount(0x7ffeebfe66b0, cash $100)

# create locked account2 and try to deposit into it; this should fail
new cash BankAccount(0x7ffeebfe6670, cash $0)
delete account BankAccount(0x7ffeebfe6670, cash $0)
# FAILED: account2 deposit failed!: tried to write to a locked (const) account

# create locked account3 and try to deposit into it; this should fail
new cash BankAccount(0x7ffeebfe6630, cash $0)
delete account BankAccount(0x7ffeebfe6630, cash $0)
...
# NOTE: make_unique creates a new ptr and will invoke foo1's copy constructor:
new Foo(0x7ffee95e2088, data=foo1)
copy constructor Foo(0x7fe5a84029e0, data=)
delete Foo(0x7ffee95e2088, data=foo1)

# NOTE: to avoid the copy, do this:
new Foo(0x7fe5a8402a00, data=foo2)

# As you cannot copy unique pointers, reassign it with move

# Let's print all the unique ptrs now
uptr1 = Foo(0x7fe5a84029e0, data=foo1)
uptr2 = nullptr
uptr3 = Foo(0x7fe5a8402a00, data=foo2)

# Expect the unique ptr data to be destroyed now
delete Foo(0x7fe5a8402a00, data=foo2)
delete Foo(0x7fe5a84029e0, data=foo1)
</pre>
...
# Create a copy constructed class and share it between two pointers:
new Foo(0x7ffeed668220, data=foo1)
copy constructor Foo(0x7f8ffe4029f8, data=)
delete Foo(0x7ffeed668220, data=foo1)
...
delete Foo(0x7f8ffe4029f8, data=foo1)
sptr2 ref count now 0

# You can also create shared pointers WITHOUT copy constructor overhead
new Foo(0x7f8ffe4029b0, data=foo0)
sptr0 = Foo(0x7f8ffe4029b0, data=foo0)
delete Foo(0x7f8ffe4029b0, data=foo0)
</pre>
...
# create a class and share it between two pointers:
new Foo(0x7ffee90b4658, data=foo1-data)
[foo1]: MySharedPtr::make_shared MySharedPtr(0x7ffee90b4688,Foo(0x7fb4dfc029c8, data=foo1-data))
delete Foo(0x7ffee90b4658, data=foo1-data)
sptr1 ref count now 1
sptr2 ref count now 2

# release the shared sptrs, expect foo1 to be destroyed:
[foo1]: MySharedPtr::reset MySharedPtr(0x7ffee90b4688,Foo(0x7fb4dfc029c8, data=foo1-data))
sptr1 ref count now 0
[foo1]: MySharedPtr::reset MySharedPtr(0x7ffee90b4608,Foo(0x7fb4dfc029c8, data=foo1-data))
delete Foo(0x7fb4dfc029c8, data=foo1-data)
sptr2 ref count now 0
[foo1]: MySharedPtr::delete MySharedPtr(0x7ffee90b4608)
[foo1]: MySharedPtr::delete MySharedPtr(0x7ffee90b4688)
</pre>
