...
# Create a custom vector class:
new MyVector(0x7ffee4667730, currlen=0, maxlen=1 elems=[])
push_back called MyVector(0x7ffee4667730, currlen=1, maxlen=1 elems=[10])
push_back called MyVector(0x7ffee4667730, currlen=2, maxlen=2 elems=[10,11])
push_back called MyVector(0x7ffee4667730, currlen=3, maxlen=4 elems=[10,11,12])
...
# End, expect vec1 destroy:
delete MyVector(0x7ffee4667730, currlen=3, maxlen=4 elems=[10,11,12])
</pre>
How to use std::move
====================
...
# Create a custom vector class:
new MyVector(0x7f88eac029b0, currlen=0, maxlen=1 elems=[])
push_back called MyVector(0x7f88eac029b0, currlen=1, maxlen=1 elems=[10])
push_back called MyVector(0x7f88eac029b0, currlen=2, maxlen=2 elems=[10,11])
vec1: [10][11]

# Create a new copy of vec1, vec2 via copy constructor (&):
copy constructor called for MyVector(0x7f88eac029b0, currlen=2, maxlen=2 elems=[10,11])
copy constructor result is  MyVector(0x7ffee1b39698, currlen=2, maxlen=2 elems=[10,11])
vec2: [10][11]

# Check we can append onto the copied vector:
push_back called MyVector(0x7ffee1b39698, currlen=3, maxlen=4 elems=[10,11,12])
push_back called MyVector(0x7ffee1b39698, currlen=4, maxlen=4 elems=[10,11,12,13])
vec2: [10][11][12][13]

# Create a new vector from vec1, vec3 via the move constructor (&&):
std::move called for MyVector(0x7f88eac029b0, currlen=2, maxlen=2 elems=[10,11])
std::move result is  MyVector(0x7ffee1b39678, currlen=2, maxlen=2 elems=[10,11])
vec3: [10][11]

# Check we can append onto the std:move'd vector:
push_back called MyVector(0x7ffee1b39678, currlen=3, maxlen=4 elems=[10,11,14])
push_back called MyVector(0x7ffee1b39678, currlen=4, maxlen=4 elems=[10,11,14,15])
vec3: [10][11][14][15]

# Destroy the old vector, vec1. It has no invalid elems:
delete MyVector(0x7f88eac029b0, currlen=0, maxlen=0 elems=[])

# End, expect vec2 and vec3 destroy:
delete MyVector(0x7ffee1b39678, currlen=4, maxlen=4 elems=[10,11,14,15])
delete MyVector(0x7ffee1b39698, currlen=4, maxlen=4 elems=[10,11,12,13])
</pre>
...
# create account1 and try to deposit into it
new cash BankAccount(0x7ffee3b476b0, cash $0)
deposit cash called BankAccount(0x7ffee3b476b0, cash $100)
BankAccount(0x7ffee3b476b0, cash $100)
# account1 deposit succeeded!
delete account BankAccount(0x7ffee3b476b0, cash $100)

# create locked account2 and try to deposit into it; this should fail
new cash BankAccount(0x7ffee3b47670, cash $0)
delete account BankAccount(0x7ffee3b47670, cash $0)
# account2 deposit failed!: tried to write to a locked (const) account

# create locked account3 and try to deposit into it; this should fail
new cash BankAccount(0x7ffee3b47630, cash $0)
delete account BankAccount(0x7ffee3b47630, cash $0)
...
# NOTE: make_unique creates a new ptr and will invoke foo1's copy constructor:
new Foo(0x7ffee26f4088, data=foo1)
copy constructor Foo(0x7fa7e54029e0, data=)
delete Foo(0x7ffee26f4088, data=foo1)

# NOTE: to avoid the copy, do this:
new Foo(0x7fa7e5402a00, data=foo2)

# As you cannot copy unique pointers, reassign it with move

# Let's print all the unique ptrs now
uptr1 = Foo(0x7fa7e54029e0, data=foo1)
uptr2 = nullptr
uptr3 = Foo(0x7fa7e5402a00, data=foo2)

# Expect the unique ptr data to be destroyed now
delete Foo(0x7fa7e5402a00, data=foo2)
delete Foo(0x7fa7e54029e0, data=foo1)
</pre>
...
# Create a copy constructed class and share it between two pointers:
new Foo(0x7ffee5e74220, data=foo1)
copy constructor Foo(0x7f9d4bd028b8, data=)
delete Foo(0x7ffee5e74220, data=foo1)
...
delete Foo(0x7f9d4bd028b8, data=foo1)
sptr2 ref count now 0

# You can also create shared pointers WITHOUT copy constructor overhead
new Foo(0x7f9d4bd02870, data=foo0)
sptr0 = Foo(0x7f9d4bd02870, data=foo0)
delete Foo(0x7f9d4bd02870, data=foo0)
</pre>
...
# create a class and share it between two pointers:
new Foo(0x7ffee0109658, data=foo1-data)
[foo1]: MySharedPtr::make_shared MySharedPtr(0x7ffee0109688,Foo(0x7fa0364029c8, data=foo1-data))
delete Foo(0x7ffee0109658, data=foo1-data)
sptr1 ref count now 1
sptr2 ref count now 2

# release the shared sptrs, expect foo1 to be destroyed:
[foo1]: MySharedPtr::reset MySharedPtr(0x7ffee0109688,Foo(0x7fa0364029c8, data=foo1-data))
sptr1 ref count now 0
[foo1]: MySharedPtr::reset MySharedPtr(0x7ffee0109608,Foo(0x7fa0364029c8, data=foo1-data))
delete Foo(0x7fa0364029c8, data=foo1-data)
sptr2 ref count now 0
[foo1]: MySharedPtr::delete MySharedPtr(0x7ffee0109608)
[foo1]: MySharedPtr::delete MySharedPtr(0x7ffee0109688)
</pre>
